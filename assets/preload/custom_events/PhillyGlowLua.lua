...
function onCreate()
	...
	precacheImage("gradient")
	precacheImage("particle")
...
local function setColors(color, darkColor)
	if (inPhilly) then
		...
		for i = getObjectOrder("phillyGlowGradient") + 1, getProperty("members.length") - 1 do
			...
			if (
				not key:endsWith("particle.png") and
				(
...
if (true) then
	...
	function spawnPhillyGlowParticle(x, y, color)
		...
		if (recycling) then
			...
			makeQSprite(spr, "particle", x, y, getRandomFloat(.3, .75) * s, getRandomFloat(.65, .75) * s, true)
		end
...
if (true) then
	...
	function makePhillyGlowGradient(x, y)
		...
		makeQSprite("phillyGlowGradient", "gradient", x, y, 0, .75)
